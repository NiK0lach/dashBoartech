...
return(
    <div className='my-6'>
      <h2 className='text-xl font-bold mb-4 dark:text-lime-400 text-lime-800'>Order Summary Details</h2>
       {cartItems.map((cartItem, i) => {
          return(
          <div key={i} className='flex items-center justify-between text-slate-600 dark:text-slate-200 pb-3 font-semibold text-sm'>
                <div className="flex items-center gap-3">
                  <Image src={cartItem.imageUrl} alt={cartItem.title} className='rounded-xl w-14 h-14' width={249} height={249} />
                      <div className="flex flex-col">
                        <h2>{cartItem.title}</h2>
                      </div>
                </div>
                <div className='flex items-center justify-between gap-3 rounded-xl border border-gray-400'>
                    <p className='flex items-center px-6 py-3'>{cartItem.qty}</p>
                </div>
         ...
         <button onClick={handlePrevious} type='button' 
          ...
          focus:ring-lime-200 dark:focus:ring-lime-900 hover:bg-slate-800 dark:bg-lime-600 dark:hover:bg-lime-700'>
            ...
            {
              loading?(
                <button disabled className='inline-flex items-center px-6 py-3 mt-4
                sm:mt-6 text-sm font-medium text-center text-white bg-slate-900 rounded-lg focus:ring-4
                focus:ring-lime-200 dark:focus:ring-lime-900 hover:bg-slate-800 dark:bg-lime-600 dark:hover:bg-lime-700'>
